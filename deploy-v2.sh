...
# Function to delete all stacks and clean up resources
delete_stacks() {
    ...
    # Step 1: Check if main stack exists and get its status
    log_info "Checking if main stack exists: $STACK_NAME"
    local stack_status=""
    local stack_exists=false
    ...
    if [ "$stack_status" != "DOES_NOT_EXIST" ] && [ -n "$stack_status" ]; then
        stack_exists=true
        log_info "Main stack found with status: $stack_status"
    else
        log_info "Main stack does not exist: $STACK_NAME"
    fi
    
    # Step 2: Get resource information before deletion
    local frontend_bucket=""
    local cloudfront_distribution_id=""
    
    if [ "$stack_exists" = true ]; then
        log_info "Retrieving resource information before deletion..."
        ...
        log_info "Frontend bucket: ${frontend_bucket:-'Not found'}"
        log_info "CloudFront distribution: ${cloudfront_distribution_id:-'Not found'}"
    fi
    
    # Step 3: Empty frontend S3 bucket first (required for stack deletion)
    if [ -n "$frontend_bucket" ] && [ "$frontend_bucket" != "None" ]; then
        log_info "Emptying frontend S3 bucket: $frontend_bucket"
        
        # Comprehensive S3 cleanup - delete all versions and delete markers
        log_info "Deleting all object versions and delete markers..."
        
        # Method 1: Delete all versions using list-object-versions
        aws s3api list-object-versions --bucket "$frontend_bucket" --region "$REGION" --output json 2>/dev/null | \
            jq -r '.Versions[]?, .DeleteMarkers[]? | select(. != null) | "\(.Key)\t\(.VersionId)"' 2>/dev/null | \
        ...
        # Method 2: Force delete any remaining objects
        log_info "Force removing any remaining objects..."
        aws s3 rm "s3://$frontend_bucket" --recursive --region "$REGION" 2>/dev/null || true
        
        # Method 3: Use sync with empty directory as final cleanup
        log_info "Final cleanup using sync method..."
        local temp_empty_dir="/tmp/empty_dir_$$"
        mkdir -p "$temp_empty_dir"
        aws s3 sync "$temp_empty_dir" "s3://$frontend_bucket" --delete --region "$REGION" 2>/dev/null || true
        rm -rf "$temp_empty_dir"
        
        log_success "Frontend S3 bucket emptied"
    fi
    
    # Step 4: Delete main stack (this will handle nested stack deletion automatically)
    if [ "$stack_exists" = true ]; then
        case "$stack_status" in
            "DELETE_IN_PROGRESS")
                log_warning "Stack deletion already in progress, monitoring..."
                ;;
            "CREATE_FAILED"|"ROLLBACK_COMPLETE"|"DELETE_FAILED")
                log_warning "Stack is in failed state: $stack_status"
                log_info "Attempting to delete failed stack..."
                aws cloudformation delete-stack --stack-name "$STACK_NAME" --region $REGION
                ;;
            *)
                log_info "Deleting main stack: $STACK_NAME"
                aws cloudformation delete-stack --stack-name "$STACK_NAME" --region $REGION
                ;;
        esac
        
        # Wait for stack deletion (this may take a while)
        log_info "Waiting for stack deletion to complete (this may take several minutes)..."
        log_info "The stack will automatically delete all nested resources including:"
        log_info "  - CloudFront distribution (may take 15+ minutes to disable and delete)"
        log_info "  - Lambda functions and layers"
        ...
        # Use longer timeout for stack deletion (CloudFormation handles nested deletions)
        if aws cloudformation wait stack-delete-complete --stack-name "$STACK_NAME" --region $REGION 2>/dev/null; then
            log_success "Main stack deleted successfully: $STACK_NAME"
        else
            log_error "Stack deletion failed or timed out"
            log_warning "Common reasons for stack deletion failure:"
            log_warning "  1. CloudFront distribution takes 15+ minutes to delete"
            log_warning "  2. S3 buckets not empty (should be handled automatically)"
            log_warning "  3. Lambda functions have active connections"
            log_warning ""
            log_warning "Check the CloudFormation console for detailed error information:"
            log_warning "https://console.aws.amazon.com/cloudformation/home?region=$REGION#/stacks/events?stackId=$STACK_NAME"
            log_warning ""
            log_warning "If the stack is stuck, you may need to:"
            log_warning "  1. Wait longer for CloudFront distribution deletion"
            log_warning "  2. Delete resources manually that are blocking stack deletion"
            log_warning "  3. Contact AWS support for assistance"
            return 1
        fi
    fi
    ...
    if [ -n "$suffix_layers" ]; then
        for layer_name in $suffix_layers; do
            log_info "Deleting all versions of layer: $layer_name"
            local layer_versions=$(aws lambda list-layer-versions \
                --layer-name "$layer_name" \
                --region "$REGION" \
            ...
            if [ -n "$layer_versions" ]; then
                ...
                log_success "Deleted layer: $layer_name"
            fi
        done
    else
        log_info "No Lambda layers found for suffix: $SUFFIX"
    fi
    
    echo
    log_success "🎉 DELETION COMPLETED SUCCESSFULLY"
    log_success "All AWS Ops Wheel v2 resources for suffix '$SUFFIX' have been removed"
    log_success "Your AWS account has been cleaned up"
    echo
    
    log_info "Final cleanup summary:"
    log_info "  ✅ Main stack and all nested resources deleted"
    log_info "  ✅ S3 buckets emptied and deleted"
    log_info "  ✅ Lambda layers cleaned up"
    log_info "  ✅ Local build artifacts removed"
    log_info "  ✅ Templates bucket deleted"
    echo
}
